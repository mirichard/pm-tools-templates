...
4. Laboratory Buyer reviews and edits the Shopping Cart using keyboard navigation; System associates each editable field with its visible label and displays validation errors next to the affected field.
...
- If Order Reviewer returns an Order, Laboratory Buyer corrects the named fields and resubmits it; System preserves the prior review decision and records the correction in the Audit Record.
