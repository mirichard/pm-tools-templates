...
export type ToolbarProps = {
  ...
  onDiffRequest: (a: Version, b: Version) => void;
};

export function Toolbar({ markdown, templateKey, data, onLoadVersion, onDiffRequest }: ToolbarProps) {
  ...
  return (
    <div className="toolbar" aria-label="Actions">
      ...
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        ...
        <button onClick={() => {
          const a = versions.find(x=>keyOf(x)===vA);
          const b = versions.find(x=>keyOf(x)===vB);
...
function storageKey(templateKey: string) { return `pmtools:versions:${templateKey}`; }
