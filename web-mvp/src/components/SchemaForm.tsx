...
export function SchemaForm({ schema, value, onChange }: Props) {
  ...
  const runValidate = (updated: Record<string, unknown>) => {
  ...
  const renderField = (path: string[], key: string, def: any) => {
    ...
    const describedBy = (byFieldRef()?.[fieldKey]?.length ? errId : undefined);
      ...
      if (items.type === 'string') {
        return (
          <div key={id}>
            ...
            <textarea id={id} rows={4} aria-required={isRequired} aria-invalid={hasFieldError(fieldKey)} aria-describedby={describedBy}
              value={Array.isArray(val) ? (val as string[]).join('\n') : ''}
              onChange={(e) => updateValue(fullPath, e.target.value.split(/\r?\n/).filter(Boolean))} />
    ...
    if (def.enum) {
      return (
        <div key={id}>
          <label htmlFor={id}>{label}{isRequired ? ' *' : ''}</label>
          <select id={id} aria-required={isRequired} aria-invalid={hasFieldError(fieldKey)} aria-describedby={describedBy}
    ...
    if (def.type === 'number' || def.type === 'integer') {
      return (
        <div key={id}>
          <label htmlFor={id}>{label}{isRequired ? ' *' : ''}</label>
          <input id={id} type="number" aria-required={isRequired} aria-invalid={hasFieldError(fieldKey)} aria-describedby={describedBy}
            value={(val as any) ?? ''}
    ...
    // Strings (textarea heuristic)
    const isTextarea = (def.type === 'string') && (label.toLowerCase().includes('scope') || label.toLowerCase().includes('purpose') || label.toLowerCase().includes('criteria') || (def.maxLength ?? 0) > 120);
    return (
      <div key={id}>
        <label htmlFor={id}>{label}{isRequired ? ' *' : ''}</label>
        {isTextarea ? (
          <textarea id={id} rows={5} aria-required={isRequired} aria-invalid={hasFieldError(fieldKey)} aria-describedby={describedBy}
            value={(val as any) ?? ''} onChange={(e) => updateValue(fullPath, e.target.value)} />
        ) : (
          <input id={id} type="text" aria-required={isRequired} aria-invalid={hasFieldError(fieldKey)} aria-describedby={describedBy}
            value={(val as any) ?? ''} onChange={(e) => updateValue(fullPath, e.target.value)} />
        )}
        {renderErrors(fieldKey, errId)}
      </div>
