...
[Risk-based scaling (#747)](risk-based-scaling.md) defines five exposure dimensions and adjusts artifacts and review intensity while retaining this baseline. [Event-driven controls (#748)](event-driven-controls.md) define additional triggers, response ownership and notification patterns; they supplement scheduled reviews and cannot remove baseline gates. The [compliance integration framework](compliance-integration-framework.md) is a supporting reference; the accountable control owner determines the actual mandatory controls.
