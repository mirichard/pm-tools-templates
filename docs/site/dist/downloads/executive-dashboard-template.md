...
- systems-thinking
principle_rationale: Connects stakeholder needs to value metrics and systemic project
  performance.
---
...
DIVIDE(
    COUNTROWS(
        FILTER(Projects, Projects[Status] = "Completed" && Projects[EndDate] <= Projects[EndDate])
    ),
...
SUMX(
    Projects,
    Projects[Budget] * RELATED(FinancialMetrics[ROI])
) / SUM(Projects[Budget])
